...
def plot_all_transfer_functions(output_dir='output', save_plot=True):
    """
    Plot all transfer functions from the output directory.
    ...
    print(f"Found {len(tk_files)} transfer function files")
    ...
    for filename in sorted(tk_files):
        ...
        # Plot the total matter transfer function (d_tot)
        color = colors.get(params['interaction_type'], 'black')
        plt.semilogx(k, transfer_functions['d_tot'], 
    ...
    plt.ylabel('Transfer Function T(k)', fontsize=12)
    plt.title('Dark Matter Transfer Functions from CLASS\n(Interacting DM with varying masses and cross sections)', 
              fontsize=14, pad=20)
...
def plot_by_parameter(output_dir='output', parameter='mass', save_plot=True):
    """
    Plot transfer functions grouped by a specific parameter.
    ...
    for i, (key, files) in enumerate(sorted(groups.items())):
        ...
        for filename, params in files:
            ...
            color = colors.get(params['interaction_type'], 'black')
            ax.semilogx(k, transfer_functions['d_tot'], 
                       color=color, alpha=0.7, linewidth=1.0)
        
        ax.set_xlabel('k [h/Mpc]')
        ax.set_ylabel('Transfer Function T(k)')
...
if __name__ == "__main__":
    ...
    # Also create grouped plots
    print("\nCreating grouped plots...")
    plot_by_parameter(parameter='mass')
    plot_by_parameter(parameter='cross_section')
