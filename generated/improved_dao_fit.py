#!/usr/bin/env python3
"""
Improved DAO fitting with better priors and bounds.
"""
...
def improved_dao_fit():
    """Improved DAO fitting with better priors."""
    ...
    # Improved initial guesses with softer slopes and larger DAO amplitude
    initial_guesses = [
        # Guess 1: Softer envelope, larger DAO
        [0.2, 2.0, -0.5, 1.0, 2.0, 0.3, 0.0, 2.0],
        # Guess 2: Even softer envelope
        [0.15, 1.5, -0.3, 0.8, 1.5, 0.2, 0.0, 1.5],
        # Guess 3: Moderate parameters
        [0.25, 2.5, -0.6, 1.2, 2.5, 0.4, 0.5, 2.5],
        # Guess 4: Different oscillation scale
        [0.18, 1.8, -0.4, 2.0, 3.0, 0.25, 1.0, 2.0],
        # Guess 5: Conservative but with larger A
        [0.12, 1.2, -0.2, 0.6, 1.0, 0.15, 0.0, 1.0],
    ]
    
    # Improved bounds - allow softer slopes and larger DAO amplitude
    bounds = (
        [0.01, 0.5, -2.0, 0.1, 0.1, 0.0, -np.pi, 0.1],  # Lower bounds
        [1.0, 5.0, 0.5, 10.0, 10.0, 2.0, np.pi, 5.0]    # Upper bounds
    )
    
    print("\nImproved bounds:")
    print("α: [0.01, 1.0] (envelope scale)")
    print("β: [0.5, 5.0] (envelope power - softer than before)")
    print("γ: [-2.0, 0.5] (envelope power - less negative)")
    print("r_s: [0.1, 10.0] (sound horizon)")
    print("k_D: [0.1, 10.0] (damping scale)")
    print("A: [0.0, 2.0] (DAO amplitude - much larger)")
    print("φ: [-π, π] (phase)")
    print("m: [0.1, 5.0] (damping power)")
    ...
    print(f"\nBest improved DAO fit R² = {best_r2:.4f}")
    ...
    plt.title(f'Improved DAO Fit: {os.path.basename(sample_file)}')
    plt.legend()
    ...
    # Add parameter text
    param_text = f'Improved DAO Parameters:\nα={best_params[0]:.3f}, β={best_params[1]:.3f}, γ={best_params[2]:.3f}\nr_s={best_params[3]:.3f}, k_D={best_params[4]:.3f}, A={best_params[5]:.3f}\nφ={best_params[6]:.3f}, m={best_params[7]:.3f}'
    
    plt.figtext(0.02, 0.02, param_text, fontsize=10, fontfamily='monospace',
                bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.8))
    
    plt.tight_layout()
    plt.savefig('improved_dao_fit.png', dpi=300, bbox_inches='tight')
    print("Improved DAO fit saved as improved_dao_fit.png")
    plt.show()
    
    return best_params, best_r2

if __name__ == "__main__":
    print("Improved DAO fitting with better priors...")
    print("=" * 60)
    
    result = improved_dao_fit()
    
    if result is not None:
        params, r2 = result
        print(f"\nImproved DAO fitting completed!")
        print(f"Best R² = {r2:.4f}")
        print("This should have softer slopes and larger DAO amplitude!")
