#!/usr/bin/env python3
"""
Implementation of the power spectrum ratio model for DAOs.
...
def test_power_spectrum_ratio_model():
    """Test the power spectrum ratio model."""
    
    # Load data
    print("Loading data...")
    k_lcdm, d_tot_lcdm = load_transfer_function_data('output/pk-lcdm_tk.dat')
    
    # Use the oscillatory file we found
    sample_file = 'output/pk_m4.6e-04_sig1.0e-25_np2_tk.dat'
    print(f"Testing power spectrum ratio model on: {os.path.basename(sample_file)}")
    ...
    print(f"R std: {np.std(R_filtered):.4f}")
    ...
    # Parameter bounds
    bounds = (
        [0.001, 0.1, -2.0, 0.01, 0.01, 0.001, 0.01, 0.1, -np.pi, 0.01, -1.0],  # Lower bounds
        [1.0, 5.0, 0.0, 2.0, 1.0, 2.0, 10.0, 10.0, np.pi, 10.0, 1.0]       # Upper bounds
    )
    
    print("\nPower spectrum ratio model parameters:")
    ...
    for i, initial_guess in enumerate(initial_guesses):
        ...
        try:
            popt, pcov = curve_fit(power_spectrum_ratio_model, k_filtered, R_filtered, 
                                  p0=initial_guess, bounds=bounds, maxfev=50000)
            
            # Calculate R²
            R_pred = power_spectrum_ratio_model(k_filtered, *popt)
            ...
            print(f"             k_star={popt[3]:.3f}, δ={popt[4]:.3f}, A_0={popt[5]:.3f}")
    ...
    print(f"\nBest power spectrum ratio model fit R² = {best_r2:.4f}")
    ...
    R_smooth = power_spectrum_ratio_model(k_smooth, *best_params)
    plt.semilogx(k_smooth, R_smooth, 'r-', linewidth=2.5, 
                label=f'Power Ratio Model (R² = {best_r2:.4f})')
    
    plt.axhline(y=1, color='black', linestyle='--', alpha=0.5, linewidth=1, label='ΛCDM')
    plt.xlabel('k [h/Mpc]')
    plt.ylabel('R(k) = P_IDM(k) / P_ΛCDM(k)')
    plt.title(f'Power Spectrum Ratio: {os.path.basename(sample_file)}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Components breakdown
    alpha, beta, gamma, k_star, delta, A_0, k_D, m, phi_0, r_s, epsilon = best_params
    ...
    # Plot residuals
    plt.subplot(2, 3, 6)
    R_pred = power_spectrum_ratio_model(k_filtered, *best_params)
    ...
    # Add parameter text
    param_text = f'Power Spectrum Ratio Model:\nα={alpha:.3f}, β={beta:.3f}, γ={gamma:.3f}\nk_star={k_star:.3f}, δ={delta:.3f}, A_0={A_0:.3f}\nk_D={k_D:.3f}, m={m:.3f}, φ_0={phi_0:.3f}\nr_s={r_s:.3f}, ε={epsilon:.3f}'
    
    plt.figtext(0.02, 0.02, param_text, fontsize=9, fontfamily='monospace',
                bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
    
    plt.tight_layout()
    plt.savefig('power_spectrum_ratio_model_fit.png', dpi=300, bbox_inches='tight')
    print("Power spectrum ratio model fit saved as power_spectrum_ratio_model_fit.png")
    plt.show()
    
    return best_params, best_r2

if __name__ == "__main__":
    print("Testing the power spectrum ratio model...")
    print("=" * 60)
    
    result = test_power_spectrum_ratio_model()
    
    if result is not None:
        params, r2 = result
        print(f"\nPower spectrum ratio model testing completed!")
        print(f"Best R² = {r2:.4f}")
        print("This model works in log space and should better capture DAOs!")
