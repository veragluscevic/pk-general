#!/usr/bin/env python3
"""
Soft DAO fitting with constraints to prevent sharp cutoffs.
"""
...
def soft_dao_fit():
    ...
    # Soft initial guesses that avoid sharp cutoffs
    initial_guesses = [
        # Guess 1: Very soft envelope, large DAO
        [0.2, 0.5, -0.2, 1.0, 2.0, 0.3, 0.0, 2.0],
        # Guess 2: Soft envelope
        [0.15, 0.8, -0.3, 0.8, 1.5, 0.2, 0.0, 1.5],
        # Guess 3: Moderate soft parameters
        [0.25, 1.0, -0.4, 1.2, 2.5, 0.4, 0.5, 2.5],
        # Guess 4: Different scales with large A
        [0.18, 0.6, -0.25, 2.0, 3.0, 0.5, 1.0, 2.0],
        # Guess 5: Conservative but large A
        [0.12, 0.7, -0.15, 0.6, 1.0, 0.25, 0.0, 1.0],
        # Guess 6: Even softer
        [0.3, 0.4, -0.1, 1.5, 2.0, 0.4, 0.0, 1.5],
    ]
    
    # Bounds that prevent sharp cutoffs
    bounds = (
        [0.01, 0.1, -2.0, 0.01, 0.01, 0.0, -np.pi, 0.01],  # Lower bounds
        [1.0, 3.0, -0.01, 20.0, 20.0, 2.0, np.pi, 10.0]   # Upper bounds
    )
    
    print("\nSoft bounds (preventing sharp cutoffs):")
    print("α: [0.01, 1.0] (envelope scale)")
    print("β: [0.1, 3.0] (envelope power - soft, max 3.0)")
    print("γ: [-2.0, -0.01] (envelope power - soft, not too negative)")
    print("r_s: [0.01, 20.0] (sound horizon)")
    print("k_D: [0.01, 20.0] (damping scale)")
    print("A: [0.0, 2.0] (DAO amplitude)")
    print("φ: [-π, π] (phase)")
    ...
    print(f"\nBest soft DAO fit R² = {best_r2:.4f}")
