#!/usr/bin/env python3
"""
Ultra-deep trough model that can reach very small values.
...
This form allows the oscillation term to be much larger than the envelope,
enabling deep troughs that can reach very small values.
"""
...
def ultra_deep_trough_model(k, alpha, beta, gamma, k_star_factor, delta, A_0, k_D, m, phi_0, r_s, epsilon):
    """
    Ultra-deep trough model that can reach very small values.
    
    Uses a form that allows the oscillation term to be much larger.
    """
    k = np.asarray(k)
    ...
    # Ultra-deep trough form:
    # R = R_env + S * A * sin(Φ)
    # This allows R to go much smaller than R_env when the oscillation term is large and negative
    
    oscillation_term = S * A * np.sin(Phi)
    R = R_env + oscillation_term
...
def test_ultra_deep_trough_model():
    """Test the ultra-deep trough model."""
    ...
    print(f"Testing ultra-deep trough model on: {os.path.basename(sample_file)}")
    ...
    # Initial parameter guesses for the ultra-deep trough model
    # Allow much larger amplitudes to create ultra-deep troughs
    initial_guesses = [
        # Guess 1: Large amplitude
        [0.1, 1.5, -0.5, 1.2, 0.2, 0.5, 1.0, 2.0, np.pi/2, 1.0, 0.1],
        # Guess 2: Very large amplitude
        [0.2, 2.0, -0.3, 1.5, 0.1, 1.0, 0.8, 1.5, np.pi, 0.8, 0.05],
        # Guess 3: Different phase
        [0.15, 1.0, -0.4, 1.3, 0.15, 0.8, 1.2, 2.5, 0.0, 1.2, 0.08],
        # Guess 4: Conservative but larger
        [0.1, 1.2, -0.3, 1.1, 0.1, 0.6, 0.5, 1.0, -np.pi/2, 0.5, 0.0],
        # Guess 5: Very large amplitude
        [0.05, 3.0, -0.6, 1.4, 0.2, 1.2, 1.5, 2.0, np.pi/4, 1.5, 0.1],
    ]
    
    # Parameter bounds for ultra-deep trough model
    # Allow much larger amplitudes
    bounds = (
        [0.001, 0.1, -2.0, 0.8, 0.01, 0.1, 0.01, 0.1, -2*np.pi, 0.01, -1.0],  # Lower bounds
        [1.0, 5.0, 0.0, 3.0, 1.0, 5.0, 10.0, 10.0, 2*np.pi, 10.0, 1.0]       # Upper bounds
    )
    
    print("\nUltra-deep trough model parameters:")
    print("R = R_env + S * A * sin(Φ)")
    print("This allows R to go much smaller than R_env")
    ...
    print("A_0: oscillation amplitude (can be large)")
    ...
    for i, initial_guess in enumerate(initial_guesses):
        ...
        try:
            popt, pcov = curve_fit(ultra_deep_trough_model, k_filtered, R_filtered, 
                                  p0=initial_guess, bounds=bounds, maxfev=50000)
            
            # Calculate R²
            R_pred = ultra_deep_trough_model(k_filtered, *popt)
            ...
            # Check if model can create ultra-deep troughs
    ...
    print(f"\nBest ultra-deep trough model fit R² = {best_r2:.4f}")
    ...
    R_smooth = ultra_deep_trough_model(k_smooth, *best_params)
    plt.semilogx(k_smooth, R_smooth, 'r-', linewidth=2.5, 
                label=f'Ultra-Deep Model (R² = {best_r2:.4f})')
    ...
    plt.title(f'Ultra-Deep Trough Model: {os.path.basename(sample_file)}')
    plt.legend()
    ...
    # Plot residuals
    plt.subplot(2, 3, 6)
    R_pred = ultra_deep_trough_model(k_filtered, *best_params)
